...
class SmartGridSystem:
    def __init__(self, sim_file, solar_file, solarcost_kwh):
        ...
        self.battery = BatteryModel(capacity_kwh=12, max_charge_rate=3, max_discharge_rate=3,initial_soc=0.5)
        self.chp = CHPModel()
        self.rm = RateManager(previous_demand=self.final_df['Total_previous_demand'], base_rate=2)
        self.rate_df = self.rm.generate_rates()
        self.rm.save_to_excel('../rate_schedule.xlsx')
        print(self.rate_df)
