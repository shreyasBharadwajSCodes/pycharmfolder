...
class CustomSmartGridEnv:
    ...
    def step(self, battery_action, solar_action, chp_action, market_action):
        ...
        # Ensure market action is non-negative
        market_action_value = max(0, market_action_value)

        # Ensure CHP action is within valid range
    ...
    def _apply_battery_action(self, action):
        if action == 0:
            ...
            self.smart_grid_system.battery.charge_battery(available_energy)
        elif action == 2:
            ...
            self.smart_grid_system.battery.discharge_battery(demand_left)
    ...
    def _calculate_penalties(self):
        ...
        demand_left = total_demand - supplied_energy
        if demand_left > 0:
            penalty += demand_left * 200  # Penalty for unmet demand, increase as needed
        if supplied_energy > total_demand:
            penalty += (supplied_energy - total_demand) * 50  # Penalty for over-supply, adjust the factor as needed

        if demand_left > 0 and supplied_energy < total_demand:
            penalty += 5000  # Adjust this value as needed to make it harsher

        return penalty

    def _calculate_market_reward(self, market_action_value):
        if market_action_value > 0:
            return market_action_value * 10  # Adjust this factor as needed
        return 0
