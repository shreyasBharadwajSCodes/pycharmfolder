...
def identify_demand_periods(total_current_demand):
    """
    Identify low and high demand periods based on the demand data.
    """
    demand_threshold = total_current_demand.mean()
    low_demand_periods = total_current_demand[total_current_demand < demand_threshold].index.tolist()
    high_demand_periods = total_current_demand[total_current_demand >= demand_threshold].index.tolist()
    return low_demand_periods, high_demand_periods
...
def is_possible_action(smart_grid_system, state, action, total_demand):
    ...
    logging.info(
        f"Checking feasibility of action: {action}, Total Produced: {total_produced}, Public Grid Usage: {public_grid_usage}, Total Demand: {total_demand}")
    ...
    for t in range(start_step, end_step):
        ...
        for state, actions in beam:
            ...
            for action in possible_acts:
                if not is_possible_action(smart_grid_system, state, action,
                                          ceil(smart_grid_system.final_df['Total_current_demand'].iloc[t])):
                    continue
                next_state = transition(smart_grid_system, state, action)
                all_candidates.append((next_state, actions + [action]))

        if not all_candidates:
            logging.error(f"No feasible actions found for time step {t}.")
            return float('inf'), []
...
def transition(smart_grid_system, state, action):
    ...
    if battery_mode == 'charge':
        smart_grid_system.battery.charge(battery_rate)
    elif battery_mode == 'discharge':
        smart_grid_system.battery.discharge(battery_rate)
    ...
    total_available_energy = available_solar + action['chp_output'] + action['electric_market_purchase'] + action[
        'public grid usage']
    remaining_demand = max(0, smart_grid_system.final_df['Total_current_demand'].iloc[
        next_state['time step']] - total_available_energy)
    ...
    logging.info(
        f"Transitioned to next state: Time Step: {next_state['time step']}, Total Cost: {next_state['total_cost']}, Battery SOC: {next_state['battery'][0]}, CHP Output: {next_state['chp'][0]}, Solar Output: {available_solar}, Electric Market Purchase: {action['electric_market_purchase']}, Public Grid Usage: {action['public grid usage']}")
...
def action_cost(smart_grid_system, state, action):
    ...
    cost += smart_grid_system.pg.get_price(action['public grid usage'],
                                           smart_grid_system.final_df['Total_current_demand'].iloc[state['time step']])
    return cost


def possible_actions(smart_grid_system, state, time_step):
    actions = []
    low_demand_periods, high_demand_periods = identify_demand_periods(
        smart_grid_system.final_df['Total_current_demand'])
    battery_modes = ['idle', 'charge', 'discharge']
    chp_outputs = [0] + list(
        range(smart_grid_system.chp.min_operational_value, smart_grid_system.chp.max_operational_value + 1, 1))
    total_demand = ceil(smart_grid_system.final_df['Total_current_demand'].iloc[time_step])

    for battery_mode in battery_modes:
        battery_rate_range = range(1,
                                   smart_grid_system.battery.max_charge_rate + 1) if battery_mode == 'charge' else range(
            1, smart_grid_system.battery.max_discharge_rate + 1) if battery_mode == 'discharge' else [0]
        for battery_rate in battery_rate_range:
            for chp_output in chp_outputs:
                for electric_market_purchase in range(0, total_demand + 1, 1):
                    solar_mode = 'on' if smart_grid_system.solar_model.get_output(state['time step']) > 0 else 'off'
                    total_produced = (
                            (
                                battery_rate if battery_mode == 'discharge' else -battery_rate if battery_mode == 'charge' else 0) +
                            chp_output +
                            (smart_grid_system.solar_model.get_output(
                                state['time step']) if solar_mode == 'on' else 0) +
                            electric_market_purchase
                    )
                    ...
                    if total_produced + public_grid_usage >= total_demand:
                        ...
                        if (battery_mode == 'charge' and time_step in low_demand_periods) or (
                                battery_mode == 'discharge' and time_step in high_demand_periods):
                            actions.append(action)
                        elif battery_mode == 'idle':
                            actions.append(action)

    logging.info(f"Possible actions for time step {time_step}: {actions}")
    return actions
...
def optimize_interval(args):
    ...
    try:
        ...
        logging.info(f"Interval from {start_step} to {end_step} optimized with result {result}.")
...
def plot_graphs(actions, smart_grid_system):
    ...
    for action in actions:
        ...
        electric_market_costs.append(
            rate_df.iloc[state['time step']]['Rate'] * action['electric_market_purchase'] if state['time step'] < len(
                rate_df) else 0)
        public_grid_costs.append(smart_grid_system.pg.get_price(action['public grid usage'],
                                                                smart_grid_system.final_df['Total_current_demand'].iloc[
                                                                    state['time step']]))
...
def distribute_procurement(actions):
    ...
    for i in range(0, len(actions), billing_interval):
        ...
        if interval_actions:
            avg_purchase = sum(action['electric_market_purchase'] for action in interval_actions) / len(
                interval_actions)
...
if __name__ == "__main__":
    smart_grid_system = SmartGridSystem(sim_file=r'../../data/load_details/simulation_file_20240523_150250.xlsx',
                                        ...
                                        solarcost_kwh=0.1)
    max_timesteps = 40  # 1440 for full day
    interval_length = 20  # Example interval length

    intervals = [(smart_grid_system, i, min(i + interval_length, max_timesteps), 5, i) for i in
                 range(0, max_timesteps, interval_length)]
    ...
    with Manager() as manager:
        ...
        with Pool(processes=2) as pool:
            pool.map(optimize_interval, [(smart_grid_system, start, end, beam_width, iter_value, result_dict) for
                                         smart_grid_system, start, end, beam_width, iter_value in intervals])
